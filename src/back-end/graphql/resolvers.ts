...
export const resolvers = {
    Query: {
        me: async (_: any, __: any, { user }: any) => {
            ...
            getOnboardingApplicationByUserId: async (_: any, { userId }: { userId: string }) => {
    ...
    Mutation: {
        ...
        updateOnboardingApplication: async (_: any, { id, data }: { id: string; data: any }) => {
        ...
        updateApplicationStatus: async (_: any, { id, status, feedback }: { id: string; status: "pending" | "approved" | "rejected"; feedback?: string }) => {
            const application = await OnboardingApplication.findById(id);
            if (!application) {
                throw new Error("Onboarding application not found");
            }
        ...
        createVisaStatusManagement: async (_: any, { userId }: { userId: string }) => {
            const user = await User.findById(userId);
            if (!user) {
                throw new Error("User not found");
            }

            const existingRecord = await VisaStatusManagement.findOne({ user: userId });
            if (existingRecord) {
                throw new Error("Visa status management record already exists for this user");
            }
        ...
        updateVisaStatusStep: async (
            ...
            visaStatus[stepName] = {
                ...
                document: {
                    ...
                    uploadedAt: data.document?.uploadedAt ?? visaStatus[stepName]?.document?.uploadedAt ?? new Date(),
                },
            };
